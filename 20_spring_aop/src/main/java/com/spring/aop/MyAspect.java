...
public class MyAspect {
	...
	@Pointcut("execution(* work())") // 충복되는 메서드를 기술
	...
	public void around(ProceedingJoinPoint pjp) {
		...
		try {
			...
			// 메소드 호출 부분
			pjp.proceed(); // ProceddingJoinPoint의 proceed 메소드를 사용하여 호출된 메소드가 동작 
		
			// 메소드 호출 후
			long endTime = System.currentTimeMillis();
			System.out.println("업무 소요시간 : " + (endTime - startTime) + "초");
			System.out.println("========================");
	...
	public void afterReturning(JoinPoint jp) { // JoinPoint를 통하여 메소드의 파라메타를 전달받을 수 있다.
	...
	public void afterThrowing() {
		// System.out.println("AOP AfterThrowing 메소드 호출 : 실수로 보고서를 다른 상사에게 보냈다.");
		logger.info("AOP AfterThrowing 메소드 호출 : 실수로 보고서를 다른 상사에게 보냈다.");
	}
